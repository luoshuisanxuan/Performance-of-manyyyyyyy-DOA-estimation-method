...
%      -estSBL（SBL算法检测的入射角）: variable x 2 x sample double （分集 x 目标数 x 样本数）
clc; clear; close all;
dd = 0.5;               % space 
...
%% 设置测试信号基本参数及默认参数
snr = 0;         % 默认信噪比
sample = 150;    % 产生n个测试样本:建议1000-5000
kelm = 8;         % 默认阵列数量
snapshot = 64;     % 默认快拍数量
...
%% 产生空间谱并保存
VariableARR = 2:4:32; %SNR 如果是阵元数则VariableARR = 2:2:16;
